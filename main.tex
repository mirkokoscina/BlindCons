...
the client signature $\sigma_{c}$. Then prepares the transaction
proposal message to be sent to the endorsing peer. The transaction
proposal consists of invoking the operations $o_{bc}$ and computing
the state update denoted by \textit{stateUpdate} and version
dependency denoted by $\textit{verDep}$. The version dependencies relates the variables involve in the transaction with node. For example: if a client wants to read and write in the blockchain, the version dependency is a tuple $\left(readset,writeset\right)$ where:
